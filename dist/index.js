...
var __importDefault = (globalThis && globalThis.__importDefault) ||
    function (mod) {
        return mod && mod.__esModule ? mod : { default: mod };
    };
Object.defineProperty(exports, '__esModule', { value: true });
...
async function content_density_evaluator(parameters) {
    ...
    const paragraphTexts = paragraphHTML
        .map(stripTags)
        .filter((t) => t.length > 0);
    ...
    const paragraphWordCounts = paragraphTexts.map((p) => p.split(/\s+/).filter(Boolean).length);
...
async function accessibility_surface_check(parameters) {
    ...
    if (unlabeledButtons > 0) {
        notes.push(`${unlabeledButtons} <button> element(s) have no visible text or aria-label.`);
    }
    else {
        notes.push('All buttons appear to have labels or aria-labels ✅');
    }
    if (headingOrderIssues > 0) {
        notes.push(`${headingOrderIssues} heading level jump(s) detected (e.g. h2 → h5).`);
    }
    else {
...
async function link_health_inspector(parameters) {
    ...
    const absoluteLinks = webLinks.map(toAbsolute).filter((l) => !!l);
    // Deduplicate
    ...
    const notes = [];
    if (brokenCount > 0)
        notes.push(`${brokenCount} broken link(s) detected.`);
    if (redirectCount > 0)
        notes.push(`${redirectCount} redirecting link(s) detected.`);
    if (internalLinks.length === 0)
        notes.push('No internal links found. Is this a standalone landing page?');
    if (externalLinks.length > 0 && internalLinks.length === 0)
        notes.push('Page links mostly out to other domains.');
    if (notes.length === 0)
        notes.push('Link health looks generally OK.');
    return {
        url,
...
async function speed_heuristics_checker(parameters) {
    ...
    // too many scripts
    if (totalScripts > 10)
        perfScore -= (totalScripts - 10) * 2;
    // too many blocking
    if (blockingScripts > 5)
        perfScore -= (blockingScripts - 5) * 4;
    // heavy inline JS
    if (inlineBytes > 50000)
        perfScore -= 15; // >50KB inline
    if (inlineBytes > 150000)
        perfScore -= 20; // >150KB inline (extra hit)
...
async function getPageSize(url) {
    ...
    page.on('response', async (response) => {
        try {
            ...
            console.warn(`Failed to buffer response from ${response.url()}:`, e);
        }
    });
    try {
        // Load the page fully
...
async function getEmissions(bytes) {
    const API_URL = 'https://api.websitecarbon.com/data?bytes=' + bytes + '&green=0';
    try {
        const response = await fetch(API_URL);
        if (!response.ok) {
            throw new Error(`Failed to fetch emissions data: ${response.status}`);
        }
...
async function carbonEmissions(parameters) {
    try {
        ...
        throw new Error(`Failed to analyze carbon emissions for ${parameters.url}: ${error}`);
    }
}
// Example:
