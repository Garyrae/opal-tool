...
async function sgc_greeting(parameters: { name: string; language?: string }) {
 ...
 */
// Apply tool decorator after function definition
async function sgc_todays_Date(parameters: { format?: string }) {
 ...
 */
async function content_density_evaluator(parameters: { url: string }) {
    ...
    const paragraphWordCounts = paragraphTexts.map(
    ...
    if (avgParagraphLength > 80) {
        notes.push(
            'Paragraphs are long; consider splitting large blocks of text.'
        );
    } else {
...
async function accessibility_surface_check(parameters: { url: string }) {
    const { url } = parameters;
    // Fetch the page HTML
    const res = await fetch(url);
    if (!res.ok) throw new Error(`Failed to fetch ${url}: ${res.status}`);
    ...
    for (const block of buttonBlocks) {
        ...
        const ariaLabelMatchDouble = block.match(
            /\baria-label\s*=\s*"([^"]*)"/i
        );
        const ariaLabelMatchSingle = block.match(
            /\baria-label\s*=\s*'([^']*)'/i
        );
        ...
        if (
            (!visibleText || visibleText.length === 0) &&
    ...
    if (h1Count === 0) {
        notes.push(
            'No <h1> found — every page should have a single main heading.'
        );
    } else if (h1Count > 1) {
...
async function link_health_inspector(parameters: { url: string }) {
    const { url } = parameters;
    ...
    function pathDepth(u: string): number | null {
        try {
    ...
    if (internalLinks.length === 0)
        notes.push(
            'No internal links found. Is this a standalone landing page?'
        );
